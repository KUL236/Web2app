...
async function resolveFaviconUrl(websiteUrl) {
  ...
  if (isPrivateHost(new URL(websiteUrl).hostname)) return fallback

  try {
    const response = await fetch(websiteUrl, {
      redirect: 'follow',
      signal: AbortSignal.timeout(4000), // Netlify functions time out after ~10s
      headers: {
...
exports.handler = async (event) => {
  ...
  try {
    ...
    const cleanName = String(app_name).replace(/[\u0000-\u001f\u007f]/g, ' ').replace(/\s+/g, ' ').trim()
    if (cleanName.length < 2 || cleanName.length > 50) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'App name must be 2-50 characters' }) }
    }
    if (!/^#[0-9A-Fa-f]{6}$/.test(icon_color || '#6366f1')) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Invalid icon colour' }) }
    }
    if (package_name.length > 60) {
      return { statusCode: 400, headers, body: JSON.stringify({ error: 'Package name is too long' }) }
    }
    ...
    if (resolvedIconSource === 'upload') {
      ...
      if (!parsed || !['image/png', 'image/jpeg', 'image/jpg', 'image/webp'].includes(parsed.mimeType) || parsed.buffer.length > 4 * 1024 * 1024) {
        await adminClient.from('apps').delete().eq('id', app.id)
    ...
    await userClient.from('notifications').insert({
      user_id: user.id,
      title: 'Build Started',
      message: `Your app "${cleanName}" is being built. This usually takes 3-5 minutes.`,
      type: 'info',
    })
